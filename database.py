...
def add_config(
    ...
    with transaction() as cur:
        cur.execute(
            """INSERT INTO configs (user_id, plan, config, expiry, created_at, type, service_id, qr_file_id, source)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (user_id, plan_name, encrypted_config, expiry, _now(), config_type, service_id, qr_file_id, source),
        )
