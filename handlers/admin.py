...
async def approve_charge(callback: types.CallbackQuery):
    ...
    try:
        ...
        await send_rich_message(callback.bot, int(uid), user_text("notif_wallet_charge_approved", amount=amount))
    except Exception:
        pass
...
async def reject_charge(callback: types.CallbackQuery):
    ...
    try:
        ...
        await send_rich_message(callback.bot, int(uid), user_text("notif_receipt_rejected_short"))
    except Exception:
        pass
...
async def custom_charge_apply(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await send_rich_message(message.bot, int(uid), user_text("notif_wallet_charged", amount=amount))
    except Exception:
        pass
    await state.clear()
...
async def approve_purchase(callback: types.CallbackQuery):
    ...
    try:
        ...
        await send_rich_message(callback.bot, int(uid), _confirm_text)
    except Exception:
        pass
...
async def reject_purchase(callback: types.CallbackQuery):
    ...
    try:
        await send_notification_sticker(callback.bot, int(uid), "notif_receipt_rejected")
        await send_rich_message(callback.bot, int(uid), user_text("notif_receipt_rejected"))
    except Exception:
        pass
    await _notify_main_admin_action(callback.bot, callback.from_user, "رد رسید", uid if 'uid' in locals() else str(order_id) if 'order_id' in locals() else "", "")
    await callback.answer("❌ رد شد.")
...
async def _finalize_send(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.bot.send_photo(
            ...
            qr_file_id,
            caption=str(caption),
            caption_entities=message_entities_from_dicts(getattr(caption, "entities", None)),
...
def _guide_detail_text_and_entities(guide: dict):
    ...
    # Entityهای عنوان دقیقاً همان چیزی هستند که Telegram هنگام ارسال عنوان
    # فرستاده است؛ آن‌ها را بعد از پیشوند «📚 » با offset UTF-16 جابه‌جا می‌کنیم.
    for entity in message_entities_from_dicts(guide.get("title_entities") or []):
        try:
            entities.append(entity.model_copy(update={
                "offset": int(entity.offset) + telegram_utf16_length("📚 ")
            }))
        except Exception:
            pass
    ...
    # Telegram برای entities از offset/length برحسب UTF-16 استفاده می‌کند.
    # اگر نسخه‌ای قدیمی از راهنما entity خراب/قدیمی ذخیره کرده باشد، ارسال کل
    # راهنما نباید با ENTITY_TEXT_INVALID شکست بخورد؛ فقط همان entity نامعتبر حذف می‌شود.
    ...
    for entity in entities:
        try:
            ...
            if offset < 0 or length <= 0 or offset + length > total:
                continue
            # مرز entity نباید وسط surrogate pair باشد.
            raw = text.encode("utf-16-le")
            raw[:offset * 2].decode("utf-16-le")
            raw[:(offset + length) * 2].decode("utf-16-le")
            valid.append(entity)
        except Exception:
            continue
    return text, valid
...
async def _send_guide_detail(target: types.Message, guide_id: int):
    ...
    try:
        ...
        # اگر Telegram به‌خاطر entity قدیمی/خراب رد کرد، یک بار بدون entity
        # ارسال می‌کنیم تا خود بخش راهنما از کار نیفتد.
        await target.answer(caption, parse_mode=None, reply_markup=kb)
...
async def admin_guide_new_title(message: types.Message, state: FSMContext):
    ...
    if not title:
        ...
        return
    title_entities = serialize_message_entities(getattr(message, "entities", None))
    # Entityهای عنوان را دقیقاً همان‌طور که Telegram فرستاده ذخیره می‌کنیم.
    await state.update_data(guide_new_title=title, guide_new_title_entities=title_entities)
...
async def admin_guide_new_content(message: types.Message, state: FSMContext):
    ...
    guide = db.create_guide(
        ...
        file_id=file_id, body_entities=body_entities, title_entities=title_entities,
    )
    await state.clear()
...
async def admin_guide_edit_name_save(message: types.Message, state: FSMContext):
    ...
    db.update_guide(guide_id, title=title, title_entities=title_entities)
    await state.clear()
