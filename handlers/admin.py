...
async def _finalize_send(message: types.Message, state: FSMContext):
    ...
    admin_message_key = (
        "service_delivery_test_admin_message"
        if is_test_delivery else "service_delivery_admin_message"
    )
    admin_message = user_text(admin_message_key)
    caption = user_text(
        delivery_text_key,
    ...
    # اگر متن ذخیره‌شده از نسخه قدیمی هنوز {admin_message} را نگه داشته باشد،
    # هیچ‌وقت این placeholder را به کاربر نشان نده.
    if isinstance(caption, str) and "{admin_message}" in caption:
        caption = caption.replace("{admin_message}", str(admin_message))
