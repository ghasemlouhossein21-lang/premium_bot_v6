...
async def _send_guide_detail(target: types.Message, guide_id: int):
    ...
    try:
        if guide["content_type"] == "photo" and guide.get("file_id"):
            ...
            await answer_rich(target, caption, entities=entities or None, parse_mode=None, reply_markup=kb)
    except Exception:
        await answer_rich(target, caption, entities=entities or None, parse_mode=None, reply_markup=kb)
...
async def admin_botinfo_edit_save(message: types.Message, state: FSMContext):
    ...
    if not key or key not in labels:
        await state.clear()
        await answer_rich(message, "❌ خطای داخلی. دوباره تلاش کنید.")
        return
...
async def admin_pasargad_open(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        await edit_rich(callback.message, text, reply_markup=admin_pasargad_menu(text))
    except TelegramBadRequest:
        await answer_rich(callback.message, text, reply_markup=admin_pasargad_menu(text))
    await callback.answer()
...
async def admin_pasargad_open_msg(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    text = _pasargad_status_text()
    await answer_rich(message, text, reply_markup=admin_pasargad_menu(text))
...
async def admin_pasargad_test(callback: types.CallbackQuery):
    ...
    icon = "✅" if ok else "❌"
    await answer_rich(callback.message, f"{icon} {msg}")
...
async def admin_panel_choose_open(callback: types.CallbackQuery):
    ...
    try:
        await edit_rich(callback.message, text, reply_markup=admin_panel_choose_menu(available, active))
    except TelegramBadRequest:
        await answer_rich(callback.message, text, reply_markup=admin_panel_choose_menu(available, active))
    await callback.answer()
...
async def admin_panel_choose_open_msg(message: types.Message):
    ...
    available = vpn_panel.available_panels()
    if not available:
        await answer_rich(message, "هیچ پنل VPNی متصل نیست. ابتدا مرزبان یا پاسارگارد را وصل کنید.")
        return
    active = vpn_panel.active_panel()
    text = (
        "🔀 انتخاب پنل VPN فعال\n\n"
    ...
    await answer_rich(message, text, reply_markup=admin_panel_choose_menu(available, active))
