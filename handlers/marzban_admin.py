...
async def marzban_send_service(callback: types.CallbackQuery, state: FSMContext):
    ...
    mapping = db.get_marzban_plan_map_for_plan_key(plan_key)
    if not mapping:
        await callback.answer("❌ برای دسته‌بندی این پلن هنوز بسته‌ی مرزبان نگاشت نشده.", show_alert=True)
        return
    ...
    if not ok:
        await answer_rich(callback.message, 
            ...
            f"🔑 planSlug ارسال‌شده: <code>{html.escape(mapping['plan_slug'])}</code> "
...
async def _deliver_marzban_link(bot, ctx: dict, link: str):
    ...
    if isinstance(send_result, Exception):
        ...
        await send_rich(bot, ADMIN_ID, "✅ سرویس به‌صورت خودکار ساخته و برای کاربر ارسال شد.")
...
async def marzban_renew_volume_received(message: types.Message, state: FSMContext):
    ...
    await answer_rich(message, "تعداد روز اعتبار جدید رو وارد کن (برای همیشگی عدد 0 رو بفرست):")
...
async def marzban_renew_days_received(message: types.Message, state: FSMContext):
    ...
    if days is None:
        await answer_rich(message, "❌ یک عدد معتبر برای تعداد روز وارد کن:")
        return
    ...
    await answer_rich(message, "⏳ در حال تمدید...")
