...
from utils import answer_rich, edit_rich, message_entities_from_dicts, telegram_utf16_length
...
import bot_info
from config import ADMIN_ID, PLANS_INTRO_TEXT, DATABASE_PATH, REFERRAL_LOCK_AMOUNT
...
def _user_guide_entities(guide: dict) -> list:
    """Entityهای ذخیره‌شده‌ی بدنه را برای متن نهایی با عنوان، شیفت می‌دهد."""
    entities = message_entities_from_dicts(guide.get("body_entities") or [])
    if not entities or not guide.get("body_text"):
        return entities
...
async def menu_user_guide_open(callback: types.CallbackQuery):
    ...
    try:
        if guide["content_type"] == "photo" and guide.get("file_id"):
            await callback.message.answer_photo(guide["file_id"], caption=caption, caption_entities=_user_guide_entities(guide), reply_markup=user_guide_detail_keyboard())
        elif guide["content_type"] == "video" and guide.get("file_id"):
            await callback.message.answer_video(guide["file_id"], caption=caption, caption_entities=_user_guide_entities(guide), reply_markup=user_guide_detail_keyboard())
        else:
            await callback.message.answer(caption, entities=_user_guide_entities(guide), reply_markup=user_guide_detail_keyboard())
    except Exception:
