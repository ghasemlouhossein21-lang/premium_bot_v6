...
def _entity_span_is_valid(text: str, ent) -> bool:
    """Validate a Telegram entity against the current UTF-16 text.
    Telegram offsets/lengths are UTF-16 units and must not split a surrogate pair.
    """
    try:
    ...
    if end > total:
        return False
    # Convert boundaries to Python indices and ensure neither boundary falls
    # inside a non-BMP code point (which would split its UTF-16 surrogate pair).
...
def _sanitize_entities_for_text(text: str, entities) -> list:
    """Keep only entities whose UTF-16 spans are valid for the current text."""
    out = []
...
async def _repair_custom_emoji_entities(bot, text: str, entities):
    """Repair stored custom-emoji offsets after template rendering."""
    ...
    def py_from_u16(s, units):
        ...
        for i,ch in enumerate(s):
            ...
            if used >= units: return i+1
        return len(s)
    for ent in custom:
        sid=str(ent.custom_emoji_id); alt=cache.get(sid)
        if alt:
            hits=[]; start=0
            while True:
...
async def send_photo_rich(bot, chat_id, photo, caption=None, **kwargs):
    """Send photo/caption while preserving valid Telegram entities."""
    ...
    if entities:
        ...
        try:
            ...
            # Never let a stale entity kill delivery; retry once with only the
            # entities that are structurally valid for the current caption.
            safe = _sanitize_entities_for_text(str(caption), normalized)
            try:
                return await bot.send_photo(chat_id=chat_id, photo=photo, caption=str(caption), caption_entities=safe or None, parse_mode=None, **kwargs)
            except TelegramBadRequest as exc2:
                if "ENTITY_TEXT_INVALID" not in str(exc2).upper(): raise
