...
async def _repair_custom_emoji_entities(bot, text: str, entities):
    """Repair persisted custom-emoji offsets before sending.

    Older saves may contain an offset that no longer points exactly at the
    fallback emoji (for example after a text was edited). Telegram rejects
    those entities with ENTITY_TEXT_INVALID. We ask Telegram for the
    custom-emoji metadata, obtain its canonical fallback emoji, and move the
    entity onto the nearest occurrence of that emoji in the current text.
    """
    if not entities:
        return []
    from aiogram.types import MessageEntity
    ...
    # Cache metadata on the bot instance so a frequently shown menu does not
    # create a Telegram API request on every message.
    cache = getattr(bot, "_premium_emoji_alt_cache", None)
    if cache is None:
        cache = {}
        try:
            setattr(bot, "_premium_emoji_alt_cache", cache)
        except Exception:
            pass
    ...
    if ids:
        try:
            ...
            # If metadata cannot be fetched, keep already-valid entities and
            # drop only entities whose current span is clearly not an emoji.
            pass

    def u16(s):
        return len((s or "").encode("utf-16-le")) // 2

    def py_index_from_u16(s, units):
        used = 0
    ...
    def emoji_candidates(alt):
        # Telegram's custom emoji alt is normally a single emoji grapheme.
        # Include VS16, skin tones and ZWJ-linked sequences.
        if not alt:
            return []
        out=[]
        i=0
        while i < len(alt):
            start=i
            i += 1
            while i < len(alt) and ord(alt[i]) in (0xFE0E,0xFE0F,0x20E3,0x200D):
                i += 1
                if i < len(alt) and ord(alt[i]) in range(0x1F3FB,0x1F400):
                    i += 1
            out.append(alt[start:i])
        return out

    for ent in custom:
        sid = str(ent.custom_emoji_id)
        alt = cache.get(sid)
        old_off = int(ent.offset)
        old_len = int(ent.length)
        if alt:
            # Search exact canonical alt; prefer the occurrence closest to the
            # old UTF-16 offset so repeated emojis remain deterministic.
            hits=[]
            start=0
            while True:
            ...
            if hits:
                old_py=py_index_from_u16(text, old_off)
                pos=min(hits, key=lambda x: abs(x-old_py))
                ent=ent.model_copy(update={"offset":u16(text[:pos]), "length":u16(alt),})
                result.append(ent)
                continue
        # Fallback: only keep the entity if its existing span is emoji-like.
        start=py_index_from_u16(text, old_off)
        end=py_index_from_u16(text, old_off+old_len)
        span=text[start:end]
        if span and any(ord(ch) >= 0x1F000 or 0x2600 <= ord(ch) <= 0x27BF for ch in span):
            result.append(ent)
    return result

async def send_rich_message(bot, chat_id, value, **kwargs):
    """Send editable text while preserving and repairing Telegram entities."""
    entities = getattr(value, "entities", None) or None
    if entities:
        kwargs.pop("parse_mode", None)
        normalized = await _repair_custom_emoji_entities(bot, str(value), entities)
        try:
            return await bot.send_message(
                chat_id=chat_id, text=str(value), entities=normalized or None,
                parse_mode=None, **kwargs
            )
        except TelegramBadRequest as exc:
            if "ENTITY_TEXT_INVALID" not in str(exc).upper():
                raise
            logger.warning("Custom Emoji entity نامعتبر بود؛ پیام بدون entity ارسال شد")
...
async def answer_rich(target, value, **kwargs):
    ...
    entities = getattr(value, "entities", None) or None
    # CallbackQuery.answer is an alert/toast API and does not accept message entities.
    if not entities or not hasattr(target, "bot") or not hasattr(target, "message"):
        return await target.answer(text=str(value), **kwargs)
    normalized = await _repair_custom_emoji_entities(target.bot, str(value), entities)
    kwargs.pop("parse_mode", None)
    try:
...
async def edit_rich(target, value, **kwargs):
    ...
    entities = getattr(value, "entities", None) or None
    if entities:
        bot = getattr(getattr(target, "chat", None), "bot", None)
        # aiogram Message does not expose bot on every version; derive it from
        # the target only when available, otherwise keep stored entities.
        normalized = entities
        if bot is not None:
            normalized = await _repair_custom_emoji_entities(bot, str(value), entities)
        kwargs.pop("parse_mode", None)
...
async def show_menu_with_sticker(
    ...
    # وقتی Entityهای واقعی Telegram داریم، parse_mode نباید همزمان اعمال شود؛
    # متن و entityها منبع اصلی فرمت هستند.
    effective_parse_mode = None if entities else parse_mode
    ...
    try:
        menu_msg = await bot.send_message(chat_id=chat_id, text=str(text), reply_markup=reply_markup, parse_mode=effective_parse_mode, entities=entities or None)
        ...
        if is_message_too_long_error(e):
            ...
            try:
                menu_msg = await bot.send_message(chat_id=chat_id, text=safe_text, reply_markup=reply_markup, parse_mode=None if safe_entities else parse_mode, entities=safe_entities)
            ...
            logger.warning("ارسال منو با entity ناموفق بود؛ نسخه بدون entity ارسال می‌شود: %s", e)
            try:
